...
QGraphicsWidget *UpdaterApplet::graphicsWidget()
{
    if (!m_declarativeWidget) {
        ...
        m_declarativeWidget->engine()->rootContext()->setContextProperty("UpdaterApplet", this);
...
void UpdaterApplet::invokeBrowser(const QString& url) const
{
    if (url.isEmpty())
...
//QString UpdaterApplet::convertTemperature(KUnitConversion::UnitPtr format, QString value,
...
bool UpdaterApplet::isValidData(const QVariant &data) const
{
    return ((data != "N/A") && (!data.toString().isEmpty()));
}

void UpdaterApplet::resetPanelModel()
{
    m_panelModel.clear();
...
//void UpdaterApplet::updatePanelModel(const Plasma::DataEngine::Data &data)
//{
//    resetPanelModel();
...
//void UpdaterApplet::updateFiveDaysModel(const Plasma::DataEngine::Data &data)
//{
...
//void UpdaterApplet::updateDetailsModel(const Plasma::DataEngine::Data &data)
//{
...
void UpdaterApplet::updateNoticesModel(const Plasma::DataEngine::Data &data)
{
...
void UpdaterApplet::dataUpdated(const QString &source, const Plasma::DataEngine::Data &data)
{
    if (data.isEmpty()) {
        return;
    }
...
//    PopupApplet::dataUpdated(source, data);

    emit modelUpdated();
    update();
}

void UpdaterApplet::configAccepted()
{
    resetPanelModel();
    m_fiveDaysModel.clear();
    m_detailsModel.clear();
