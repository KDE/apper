...
KpkHistoryDelegate::KpkHistoryDelegate(QObject *parent)
 : QStyledItemDelegate(parent), m_addIcon("list-add"), m_removeIcon("list-remove")
{
//     setExtendPixmap(SmallIcon("arrow-right"));
//     setContractPixmap(SmallIcon("arrow-down"));
}

void KpkHistoryDelegate::paint(QPainter *painter,
...
int KpkHistoryDelegate::calcItemHeight(const QStyleOptionViewItem &option) const
{
...
void KpkHistoryDelegate::paintColMain(QPainter *painter,
        const QStyleOptionViewItem &option, const QModelIndex &index) const
{
...
void KpkHistoryDelegate::paintColFav(QPainter *painter,
        const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    int left = option.rect.left();
    int top = option.rect.top();
...
// QSize KpkHistoryDelegate::sizeHint(const QStyleOptionViewItem &option,
...
int KpkHistoryDelegate::columnWidth (int column, int viewWidth) const
{
    if (column != 0) {
...
// void KpkHistoryDelegate::itemActivated(const QModelIndex &index)
// {
...
#include "KpkHistoryDelegate.moc"
